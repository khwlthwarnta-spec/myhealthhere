...
function calculate(weight: number, height: number, age: number, gender: Gender): Results {
  ...
  const healthLabel = healthScore >= 80 ? "ممتاز" : healthScore >= 60 ? "جيد" : "يحتاج تحسين";
      ...
      <View style={[styles.formCard, {
        ...
        <View style={styles.genderRow}>
          {(["male", "female"] as Gender[]).map((g) => (
            ...
            >
              <MaterialCommunityIcons name={g === "male" ? "human-male" : "human-female"} size={20} color={gender === g ? "#fff" : colors.mutedForeground} />
              <Text style={[styles.genderText, { color: gender === g ? "#fff" : colors.mutedForeground, fontFamily: "Tajawal_700Bold" }]}>
                {g === "male" ? "ذكر" : "أنثى"}
              </Text>
            </Pressable>
          ))}
        ...
        <TextInput style={inputStyle} placeholder="مثال: 70" placeholderTextColor={colors.mutedForeground} keyboardType="numeric" value={weight} onChangeText={setWeight} textAlign="right" />

        <Text style={[styles.label, { color: colors.foreground, fontFamily: "Tajawal_700Bold" }]}>الطول (سم)</Text>
        <TextInput style={inputStyle} placeholder="مثال: 170" placeholderTextColor={colors.mutedForeground} keyboardType="numeric" value={height} onChangeText={setHeight} textAlign="right" />

        <Text style={[styles.label, { color: colors.foreground, fontFamily: "Tajawal_700Bold" }]}>العمر</Text>
        <TextInput style={inputStyle} placeholder="مثال: 20" placeholderTextColor={colors.mutedForeground} keyboardType="numeric" value={age} onChangeText={setAge} textAlign="right" />

        <Pressable onPress={handleCalculate} style={({ pressed }) => [styles.calcBtn, { backgroundColor: colors.primary, opacity: pressed ? 0.85 : 1 }]}>
      ...
      {results && (
        <View style={styles.results}>
          <View style={[styles.bmiCard, { backgroundColor: results.bmiColor + "15", borderColor: results.bmiColor + "40" }]}>
            <Text style={[styles.bmiLabel, { color: colors.mutedForeground, fontFamily: "Tajawal_400Regular" }]}>مؤشر كتلة الجسم</Text>
            <Text style={[styles.bmiValue, { color: results.bmiColor, fontFamily: "Tajawal_800ExtraBold" }]}>{results.bmi}</Text>
            <View style={[styles.bmiPill, { backgroundColor: results.bmiColor + "25" }]}>
              <Text style={[styles.bmiClass, { color: results.bmiColor, fontFamily: "Tajawal_700Bold" }]}>{results.bmiClass}</Text>
            </View>
          </View>

          <View style={[styles.scoreCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <Text style={[styles.scoreTitle, { color: colors.foreground, fontFamily: "Tajawal_700Bold" }]}>النقاط الصحية</Text>
            <View style={styles.scoreRow}>
              <Text style={[styles.scoreLabel, { color: colors.mutedForeground, fontFamily: "Tajawal_400Regular" }]}>{results.healthLabel}</Text>
              <Text style={[styles.scoreNum, { color: colors.primary, fontFamily: "Tajawal_800ExtraBold" }]}>{results.healthScore} / 100</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  genderBtn: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, paddingVertical: 12, borderRadius: 12, borderWidth: 1 },
  genderText: { fontSize: 14, fontWeight: "700" },
  calcBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 10, paddingVertical: 14, borderRadius: 14, marginTop: 20 },
  calcBtnText: { color: "#fff", fontSize: 16, fontWeight: "700" },
  results: { gap: 16 },
  bmiCard: { borderRadius: 20, borderWidth: 1, padding: 24, alignItems: "center" },
  ...
  statCard: { width: "47%", borderRadius: 16, borderWidth: 1, padding: 16, alignItems: "flex-end" },
